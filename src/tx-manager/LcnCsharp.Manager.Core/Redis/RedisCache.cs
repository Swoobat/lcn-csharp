...
namespace LcnCsharp.Manager.Core.Redis
{
    public class RedisCache : IDistributedCache, IDisposable
    {
        ...
        private readonly LcnCsharp.Manager.Core.Redis.RedisCacheOptions _options;
        private readonly string _instance;

        private readonly SemaphoreSlim _connectionLock = new SemaphoreSlim(initialCount: 1, maxCount: 1);

        public RedisCache(IOptions<LcnCsharp.Manager.Core.Redis.RedisCacheOptions> optionsAccessor)
        {
        ...
        public List<string> Keys(string pattern)
        {
            ...
            do
            {
                var redisResult = _cache.Execute("SCAN", new object[] { nextCursor.ToString(), "MATCH", $"{pattern}", "COUNT", "1000" });
        ...
        public void HashSet(string key, HashEntry[] hashEntry)
        {

            _cache.HashSet(key, hashEntry);
        }


        public string HashGet(string name, string key)
        {
           return  _cache.HashGet(name, key);
        }
    }
}